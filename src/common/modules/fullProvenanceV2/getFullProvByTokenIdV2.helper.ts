...
import {conditionalTitleCase} from '@/utils/case';
...
import {StaticProvContentHelper} from '../staticProvContent/staticProvContent.common.service';
...
export class GetFullProvByTokenIdHelperV2 {
  private filterInfo: {
  ...
  public async get() {
    try {
      ...
      // ------------start : detect staticdata-------

      const staticInst = new StaticProvContentHelper(
        {
          masterGtin: this.masterGtinInst?.masterVariant?.gtinKey,
          allCredTxs: this.provTxs,
          persona: this.persona,
          fssai: this.queryInput.fssai,
          signatureMasterGtins:
            this.env.SIGNAUTE_MASTER_GTINS_FOR_CUSTOM_DISPLY,
        },
        this.env.VERIFY_CRDENTIAL_CONFIG,
        this.credentialConfig,
      );
      const staticContent = await staticInst.get();
      if (staticContent) {
        this.staticContent = staticContent;
      }
      // ----end : detect staticdata----------
  ...
  private async loadFeedbackTableColDef() {
    ...
    const tableMetadataFeedback: any =
      await this.dashboardTableCommonService.getTableMetadata('Feedback');

    const isDetailsKeys: any =
      tableMetadataFeedback.tableMetadata.columnDefs.map(
        (obj: {header: any; isDetails: any; accessor: any}) => {
          if (obj?.isDetails) {
            return {accessor: obj?.accessor, header: obj?.header};
          }
          return null;
        },
      );
  ...
  private getFeedback() {
    return {
      ...
      customFeedbackKeys: this.isDetailsKeysWithoutNull,
    };
  }
  ...
  private getProvenanceProdCreds() {
    ...
    return {
      data: this.staticContent ? [] : finalCredTranArr,
      staticDynamicdata: this.staticContent?.provenances || [],
